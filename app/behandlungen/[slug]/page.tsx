...
export async function generateStaticParams() {
  ...
  return content.collections.behandlungen.map((t) => ({ slug: t.slug }));
}
...
export async function generateMetadata({
  ...
  const { content, item } = await getCollectionItem("behandlungen", slug, locale);
  if (!item) return {};
  return {
...
export default async function BehandlungPage({
  ...
  const { slug } = await params;
  const locale = await getRequestLocale();
  const { item, index } = await getCollectionItem("behandlungen", slug, locale);
  if (!item) notFound();
  const scope = `collections.behandlungen.${index}.sections`;
  return (
    <>
