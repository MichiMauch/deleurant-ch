...
export async function generateStaticParams() {
  ...
  return content.collections.ratgeber.map((r) => ({ slug: r.slug }));
}
...
export async function generateMetadata({
  ...
  const { content, item } = await getCollectionItem("ratgeber", slug, locale);
  if (!item) return {};
  return {
...
export default async function PillarPage({
  ...
  const { slug } = await params;
  const locale = await getRequestLocale();
  const { item, index } = await getCollectionItem("ratgeber", slug, locale);
  if (!item) notFound();
  const scope = `collections.ratgeber.${index}.sections`;
  return (
    <>
